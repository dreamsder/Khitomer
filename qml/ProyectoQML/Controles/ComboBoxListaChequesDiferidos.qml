...
Rectangle {
    ...
    property string codigoValorSeleccionCodigoDocumento
    property string codigoValorSeleccionTipoDocumento
    property string codigoValorSeleccionNumeroLinea
    property double opacidadPorDefecto: 0.8
    property alias colorTitulo: txtTitulo.color



    property double montoDelMedioDePagoChequeDiferido: 0.0
    ...
    function tomarElFoco(){
        if(listview1.count>=4){
            rectPrincipalComboBox.height=200
    ...
    Rectangle {
        ...
        MouseArea {
            ...
            ListView {
                ...
                anchors.fill: parent
                focus: true
                model: modeloLineasDePagoListaChequesDiferidosComboBox
                ...
                delegate:
                    ...
                    Rectangle{
                        id:rect1
                        height: texto1.implicitHeight+texto2.implicitHeight
                        ...
                        Text {
                            ...
                            Keys.onReturnPressed: {
                                ...
                                rectPrincipalComboBox.visible=false
                                txtTextoSeleccionado.text=numeroCheque+" - "+modeloListaBancos.retornaDescripcionBanco(codigoBanco)+" :"+modeloListaMonedas.retornaSimboloMoneda(monedaMedioPago)+" "+importePago
                                txtTextoSeleccionado.enabled=true

                                var agregarChequeDiferido=true;

                                for(var i=0; i<modeloListaMediosDePagoAgregados.count;i++){
                                    if(modeloListaMediosDePagoAgregados.get(i).numeroCheque==numeroCheque && modeloListaMediosDePagoAgregados.get(i).numeroBanco==codigoBanco && modeloListaMediosDePagoAgregados.get(i).tipoCheque==codigoTipoCheque){
                                        agregarChequeDiferido=false;
                                    }
                                }

                                montoDelMedioDePagoChequeDiferido=importePago;


                                if(agregarChequeDiferido){

                                    var _numeroCuentaBancaria=numeroCuentaBancaria
                                    var _codigoBancoCuentaBancaria=codigoBancoCuentaBancaria
                                    if(modeloListaTipoDocumentosComboBox.retornaPermisosDelDocumento(cbListatipoDocumentos.codigoValorSeleccion,"utilizaCuentaBancaria")){
                                        ...
                                        _numeroCuentaBancaria=cbxAfectaCuentaBancaria.codigoValorSeleccion
                                        _codigoBancoCuentaBancaria=cbxAfectaCuentaBancaria.codigoValorSeleccion2

                                    }

                                    modeloListaMediosDePagoAgregados.append({
                                                                                descripcionMedioDePago: modeloMediosDePago.retornaDescripcionMedioDePago(codigoMedioPago),
                                                                                codigoMedioDePago: codigoMedioPago,
                                                                                montoMedioDePago: importePago,
                                                                                monedaMedioPago:modeloMediosDePago.retornaMonedaMedioDePago(codigoMedioPago),
                                                                                simboloMonedaMedioDePago: modeloListaMonedas.retornaSimboloMoneda(monedaMedioPago),
                                                                                cantidadCuotas: cuotas,
                                                                                nombreTarjetaCredito:"",
                                                                                codigoTarjetaCredito:codigoTarjetaCredito,
                                                                                numeroCheque:numeroCheque,
                                                                                numeroBanco:codigoBanco,
                                                                                fechaCheque:fechaCheque,
                                                                                tipoCheque:codigoTipoCheque,
                                                                                codigoDoc:codigoDocumento,
                                                                                esDiferido:true,
                                                                                codigoTipoDoc:codigoTipoDocumento,
                                    ...
                                    if(modeloMediosDePago.retornaMonedaMedioDePago(codigoMedioPago)!=cbListaMonedasEnFacturacion.codigoValorSeleccion){

                                        if(modeloListaMonedas.retornaMonedaReferenciaSistema()==modeloMediosDePago.retornaMonedaMedioDePago(codigoMedioPago)){
                                            cotizacion=modeloListaMonedas.retornaCotizacionMoneda(cbListaMonedasEnFacturacion.codigoValorSeleccion)
                                            montoDelMedioDePagoChequeDiferido=importePago/cotizacion
                                        }else{

                                            if(cbListaMonedasEnFacturacion.codigoValorSeleccion==modeloListaMonedas.retornaMonedaReferenciaSistema()){
                                                cotizacion=modeloListaMonedas.retornaCotizacionMoneda(modeloMediosDePago.retornaMonedaMedioDePago(codigoMedioPago))
                                                montoDelMedioDePagoChequeDiferido=importePago*cotizacion
                                            }else{
                                                montoDelMedioDePagoChequeDiferido=(importePago*modeloListaMonedas.retornaCotizacionMoneda(modeloMediosDePago.retornaMonedaMedioDePago(codigoMedioPago)))/modeloListaMonedas.retornaCotizacionMoneda(cbListaMonedasEnFacturacion.codigoValorSeleccion)
                                            }
                                        }
                                    }
                                    etiquetaTotalMedioDePago.setearTotal(montoDelMedioDePagoChequeDiferido.toFixed(modeloconfiguracion.retornaValorConfiguracion("CANTIDAD_DIGITOS_DECIMALES_MONTO")),-1,codigoTipoDocumento)
                                }
        ...
        Rectangle {
            ...
            anchors.bottom: parent.bottom
            visible: {
                if(listview1.count==0 || !listview1.interactive){
                    false
                }else{
                    true
                }
